...
参数：${mysql_yesterday_l_month} = '202401'
*/

-- 2年1月份数据 84125 44000 42834 -- 83815 43690
-- 	delete from  fine_dw.dw_cb_detail
-- where sales_month =${mysql_yesterday_l_month};
...
select * from fine_dw.dw_svc_detail
 where SUBSTRING(svc_month,1,6) = ${mysql_yesterday_l_month}
--  where SUBSTRING(svc_month,1,6) = ${mysql_yesterday_l_month}
				...
				from 
		            ...
		            (SELECT item_code,bs_price,bs_starting_date,bs_ending_date,proj_name,channel
		                ...
		                FROM fine_dw.dw_item_mapping_cb
										...
										<= DATE_FORMAT(DATE_SUB(STR_TO_DATE(concat(${mysql_yesterday_l_month},'01'), '%Y%m%d'), INTERVAL 1 month), '%Y%m')
		                ) A
		                WHERE A.seq =1
...
select
		...
		sales_qty,
		service_fee,
		case when ship_to_code = '333878' OR ship_to_code = '523444' then 0 else service_fee2 end as service_fee2,  -- 服务费
