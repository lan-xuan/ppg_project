...
参数：'202209'
*/
-- TRUNCATE TABLE fine_dw.dw_customer_list;
-- insert into fine_dw.dw_customer_list 
-- (
-- 		channel,
-- 		district,
-- 		proj_name,
...
(
		...
		master1.is_flag,
		case when customer_code_2 is not null then customer_code_2 
					when master1.u_customer_code is not null then master1.u_customer_code 
					else REPLACE(t.customer_code, 'CN', '') end as u_customer_code,
		case when customer_name_2 is not null then customer_name_2 
					when master1.u_customer_name is not null then master1.u_customer_name
					else REPLACE(t.customer_code, 'CN', '')  end as u_customer_name,
		t.*
...
SELECT DISTINCT
		...
		sales_month,
		NOW()  as etl_time,
		'ods_transaction_detail_report_df' data_resource
		
		


FROM sss -- where customer_code = '212833';
   WHERE COALESCE(channel,district,proj_name,proj_name_en,district) IS NULL