-- 20240901 ： 同步7.1里的3-1公式，该表不区分channel所以只修改distributor即可
...
-- 5-1 = （1-2）/（3-1）
SELECT sales_year,sales_year as sales_month,channel,
-- sum(if(d_sales*d_volume*ly_d_sales*ly_d_volume=0,0,((d_sales/ NULLIF(d_volume, 0)) - (ly_d_sales/NULLIF(ly_d_volume, 0))*d_volume)) + if(s_sales*s_volume*ly_s_sales*ly_s_volume=0,0,(-1 * ((s_sales/NULLIF(s_volume, 0) - ly_s_sales/NULLIF(ly_s_volume, 0))) * s_volume)) ) 1-2
...
sum(if(d_sales*d_volume*ly_d_sales*ly_d_volume=0,0,((d_sales/ NULLIF(d_volume, 0)) - (ly_d_sales/NULLIF(ly_d_volume, 0))*d_volume)) + if(s_sales*s_volume*ly_s_sales*ly_s_volume=0,0,(-1 * ((s_sales/NULLIF(s_volume, 0) - ly_s_sales/NULLIF(ly_s_volume, 0))) * s_volume)) )/sum(ly_net_sales)
...
-- 		order_no,sum(sales_value)
					t.channel,
					null as customer_code,
					null as customer_name,
					null as proj_name,
					null as proj_name_en,
					case when orderno.sec_1 in ('Sales Growth%','Price Increase%') then t.sales_value*100 else t.sales_value end as sales_value,
					t.sales_month,
					t.sales_year,
					orderno.report_year,
				    orderno.order_no,
