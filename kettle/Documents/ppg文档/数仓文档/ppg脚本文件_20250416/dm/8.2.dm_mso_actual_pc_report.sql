...
where sales_month<=  ${mysql_yesterday_d_month} 
    and  sales_year = SUBSTRING(${mysql_yesterday_d_month},1,4)
),
...
SELECT
    ...
    sum(COALESCE(t.pc , 0))/sum(COALESCE(t.sales_value , 0)) as pc_rate, 
	...
	sum(COALESCE(t.adjusted_commision_fee , 0)) as adjusted_commision_fee,
	sum(COALESCE(t.adjusted_service_fee , 0)) as adjusted_service_fee,

	sum(COALESCE(t.vendor_rebate , 0)) as vendor_rebate, -- 5 ---- 总服务商返利   adjusted fee表rebate
    sum(COALESCE(t.pc , 0) - COALESCE( t.service_fee , 0) - COALESCE( t.service_rate , 0) - COALESCE( t.service_fee2 , 0) - COALESCE( t.rebate_rate , 0) - COALESCE( t.c_rebate , 0) - COALESCE( t.vendor_rebate , 0) - COALESCE( t.adjusted_fee , 0) - COALESCE( t.adjusted_service_fee, 0)) as actual_pc, -- 公式=pc-1-2-3-4-5-adjusted_service_fee
	-- MSO: actual pc% = actual_pc/(项目结算销售额-MSO集团返利-MM主机厂渠道返利-总服务商返利-调整金额）
    sum(COALESCE(t.pc , 0) - COALESCE( t.service_fee , 0) - COALESCE( t.service_rate , 0) - COALESCE( t.service_fee2 , 0) - COALESCE( t.rebate_rate , 0) - COALESCE( t.c_rebate , 0) - COALESCE( t.vendor_rebate , 0) - COALESCE( t.adjusted_fee , 0) - COALESCE( t.adjusted_service_fee, 0))/NULLIF(sum(COALESCE( sales_value  , 0) - COALESCE( t.rebate_rate , 0) - COALESCE( t.c_rebate , 0) - COALESCE( t.adjusted_fee , 0) - COALESCE( t.vendor_rebate , 0)),0) as actual_pc_rate, 
    sum(COALESCE(t.commision_fee , 0))  as commision_fee, -- 7
    sum(COALESCE(t.pc , 0) - COALESCE( t.service_fee , 0) - COALESCE( t.service_rate , 0) - COALESCE( t.service_fee2 , 0) - COALESCE( t.rebate_rate , 0) - COALESCE( t.c_rebate , 0) - COALESCE( t.vendor_rebate , 0) - COALESCE( t.adjusted_fee , 0)  - COALESCE( t.adjusted_service_fee, 0) - COALESCE( t.commision_fee ,0) - COALESCE( t.adjusted_commision_fee,0))  as ebit,  -- 公式=actual_pc-7-adjusted_commision_fee
	-- eop% = eop/(项目结算销售额-MSO集团返利-MM主机厂渠道返利-总服务商返利-调整金额）
    sum(COALESCE(t.pc , 0) - COALESCE( t.service_fee , 0) - COALESCE( t.service_rate , 0) - COALESCE( t.service_fee2 , 0) - COALESCE( t.rebate_rate , 0) - COALESCE( t.c_rebate , 0) - COALESCE( t.vendor_rebate , 0) - COALESCE( t.adjusted_fee , 0)  - COALESCE( t.adjusted_service_fee, 0) - COALESCE( t.commision_fee ,0) - COALESCE( t.adjusted_commision_fee,0))/NULLIF(sum(COALESCE( sales_value  , 0) - COALESCE( t.rebate_rate , 0) - COALESCE( t.c_rebate , 0) - COALESCE( t.vendor_rebate , 0) - COALESCE(t.adjusted_fee,0)),0) as ebit_rate, -- 公式=ebit/(sales-3-4-5）
    sum(COALESCE(t.sales_value, 0)) as sales_value,
	${mysql_yesterday_d_month} as sales_month,
    'dw_transaction_detail_report' as data_resource, 
  	SYSDATE() as etl_time,
	 
	 STR_TO_DATE(CONCAT(${mysql_yesterday_d_month} ,'01') , '%Y%m%d') as report_date
