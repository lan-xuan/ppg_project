...
SELECT sales_year,sales_month,customer_code,customer_name,proj_name,proj_name_en,channel,sum(net_sales),'GROSS SALES' as sec_1,'ACTUAL' as sec_2,upper(sales_month_en) as sec_3,'是' as is_flag FROM aa
...
SELECT sales_year-1,sales_month_2,customer_code,customer_name,proj_name,proj_name_en,channel,sum(ly_net_sales),'GROSS SALES' as sec_1,'PRIOR' as sec_2,upper(sales_month_en) as sec_3,'是' as is_flag FROM aa
...
-- 5-2 = （1-2）/（3-1）
SELECT sales_year,sales_year as sales_month,customer_code,customer_name,proj_name,proj_name_en,channel,
sum(if(COALESCE(d_volume,0)*COALESCE(ly_d_volume,0)=0,0,COALESCE(d_volume,0)*(COALESCE(d_sales/NULLIF(d_volume,0),0) - COALESCE(ly_d_sales/NULLIF(ly_d_volume,0),0))))/NULLIF(sum(ly_net_sales),0)
,'Price Increase%' as sec_1,'Price Increase‱' as sec_2,'YTD' as sec_3,null is_flag 
FROM aa WHERE 1=1 and channel <> 'DISTRIBUTOR'
GROUP BY sales_year,sales_year,customer_code,customer_name,proj_name,proj_name_en,channel
)
