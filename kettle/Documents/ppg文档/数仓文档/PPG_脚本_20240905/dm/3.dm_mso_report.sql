...
, target as(
		...
		FROM cal
		LEFT JOIN (
			SELECT sum(sales_target) sales_target,target_quarter,target_year,proj_name,target_month 
			FROM fine_dw.dw_mso_sales_target
			where 1= 1 
			and upper(report_brand_group) = 'TOTAL'
			GROUP BY target_quarter,target_year,proj_name,target_month 
			) t
...
)  -- SELECT * FROM growth_q where sales_year = ${mysql_yesterday_l_year};
...
-- SELECT order_no FROM fine_dw.dw_order_report orderno WHERE 1=1  
-- 		and orderno.order_report = 'mso_report'
-- 		and orderno.report_year =  ${mysql_yesterday_l_year} -- 具体报表的年
-- 		and order_no not in(
	SELECT  
	-- DISTINCT order_no
			t.proj_name,
			...
			orderno.sec_2,
			orderno.sec_3,
			'fine_dw.dw_transaction_detail_report/fine_dw.dw_backlog_by_product_line/fine_dw.dw_customer_master_list/fine_dw.dw_mso_sales_target/fine_dw.dw_cs_relationship_info/fine_dw.dw_order_report/fine_ods.ods_calendar_info_df' as data_resource,
			 now() as etl_time,
			 orderno.report_year,
...
-- 		and orderno.order_no = 5
		-- and  proj_name = '中升' and district ='NATIONAL' and orderno.sec_1 ='Y24 JAN'
