-- TRUNCATE TABLE fine_dm.dm_mso_service_report;
WITH  cal as 
(
	SELECT 
		DISTINCT
...
-- 		cal.proj_name,
	...
	(
		...
		WHERE channel = 'MSO' -- 'distributor' -- 'SCHOOL'-- 'MSO'
-- 		and proj_name = '中升'
...
(   
    ...
    -- and STR_TO_DATE(cs.starting_date, '%Y%m%d') <= STR_TO_DATE(CONCAT(sales_month,'01'), '%Y%m%d')
    -- and STR_TO_DATE(cs.ending_date, '%Y%m%d') >= STR_TO_DATE(CONCAT(sales_month,'01'), '%Y%m%d')
...
LEFT JOIN fine_dw.dw_order_report orderno
    ...
    and orderno.order_report = 'mso_service_report'
LEFT JOIN fine_dw.dw_cs_relationship_info cs
    on t.vendor_code = cs.customer_code
